...
### 2. `profiles`
- **SELECT:** 
  - *Public:* Anyone can read where `deleted_at IS NULL` AND `is_suspended = FALSE`.
  - *Self:* Full access (`id = auth.uid()`), even if suspended.
  - *Moderator:* Full access via JWT claim.
...
### 3. `posts`
- **SELECT:**
  - *Public:* Anyone can read where `deleted_at IS NULL`.
  - *Author:* Full access (`author_id = auth.uid()`).
...
### 7. `comments`
- **SELECT:** 
  - *Public:* Anyone where `deleted_at IS NULL`.
  - *Author/Moderator:* Full access.
...
### 11. `reports`
- **SELECT:** 
  - *Reporter:* Can view their own reports (`reporter_id = auth.uid()`).
  - *Moderator:* Full access via JWT claim.
  - *Accused:* Strictly denied.
